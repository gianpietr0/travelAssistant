...
from lib.utilities import argmax
import pandas as pd
...
class Data_set(Displayable):
    ...
    Returns a new dataset with categorical features one-hot encoded.
    """
        ...
        for example in data:
            ...
            for i, value in enumerate(example):
                if i in self.categorical_features:
                    for category in self.domains[i]:
                        new_example.append(1 if value == category else 0)
                else:
